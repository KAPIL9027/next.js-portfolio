  ...
  return (
    ...
    justify-evenly items-center md:flex-row">
      ...
      <div className=" mx-2 sm:px-0 flex flex-col space-y-4 mt-[8rem]">
        <h4 className=" text-[1.1rem] sm:text-3xl  font-semibold text-center">
        ...
        <div className="space-y-10">
            ...
            <p className="sm:text-2xl">123 Developer{"'"}s Lane</p>
            </div>
        </div>

        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col space-y-2 w-[21rem] sm:w-fit mx-auto">
            <div className="flex space-x-2 w-[20rem] sm:w-fit">
                <input {...register('name')} className="contactInput w-[10rem]" placeholder="Name" type="text"/>
                <input {...register('email')} className="contactInput w-[10.5rem]" placeholder="Email" type="email"/>
            </div>
            ...
            <button type="submit" 
            className="bg-[#F7AB0A] py-2 sm:py-5 px-10 rounded-md text-black font-bold text-lg">
                Submit
            </button>
        </form>
      </div>
